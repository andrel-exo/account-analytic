...
class TestDistributionModelDate(TransactionCase):
    ...
    def test_no_recalculate_lines_from_other_model(self):
        ...
        move.action_post()
        line = move.line_ids.filtered(lambda line: line.partner_id == self.partner_a)
        line.move_id.date = datetime.now().date() + timedelta(days=5)
        self.assertEqual(
            line.analytic_distribution, self.distribution_1.analytic_distribution
        )
        ...
        self.assertEqual(line.move_id.date, self.distribution_2.start_date)
    ...
    @freeze_time("2024-01-01")
    def test_display_name_partial_fields(self):
        record = self.env["account.analytic.distribution.model"].create(
            {"partner_id": self.partner.id}
        )
        record._compute_display_name()
        # Only partner, no dates
        self.assertIn("Acme Corp", record.display_name)
        self.assertNotIn("-", record.display_name)

    @freeze_time("2024-01-01")
    def test_get_distribution_no_match(self):
        # Should return False when no distribution model matches
        vals = {
            "product_id": False,
        ...
        result = self.env["account.analytic.distribution.model"]._get_distribution(vals)
        self.assertFalse(result)

    @freeze_time("2024-01-01")
    def test_check_score_for_dates_always_one(self):
        model = self.env["account.analytic.distribution.model"].create(
            {"start_date": Date.from_string("2024-01-01")}
        )
        self.assertEqual(model._check_score("start_date", "2024-01-01"), 1)
        self.assertEqual(model._check_score("end_date", "2024-01-01"), 1)

    @freeze_time("2024-01-01")
    def test_create_domain_for_date_and_fallback(self):
        model = self.env["account.analytic.distribution.model"]

        # Date-specific branch
        domain_date = model._create_domain("date", "2024-01-01")
        self.assertTrue(any(isinstance(x, tuple) for x in domain_date))

        # Fallback branch
        domain_other = model._create_domain("partner_id", self.partner.id)
        self.assertTrue(
            any(cond[0] == "partner_id" for cond in domain_other),
            f"Expected partner_id in domain, got {domain_other}",
        )
    ...
    def test_get_domain_variants(self):
        ...
        domain = model._get_domain()
        self.assertIn(("end_date", ">=", model.start_date), domain)
        ...
        domain = model._get_domain()
        self.assertIn(("start_date", "<=", model.end_date), domain)

        # Without account_prefix
        model = self.env["account.analytic.distribution.model"].create(
            {"partner_id": self.partner.id}
        )
        domain = model._get_domain()
        self.assertIn(("account_prefix", "=", False), domain)

    @freeze_time("2024-01-01")
    def test_get_lines_domain_variants_and_constraints(self):
        # Case with recalculate = False (should not raise, returns list)
        model = self.env["account.analytic.distribution.model"].create(
            {
                "recalculate": False,
                "partner_id": self.partner.id,
        ...
        domain = model._get_lines_domain()
        self.assertIsInstance(domain, list)

        # Case missing partner_id and account_prefix -> must raise
        model = self.env["account.analytic.distribution.model"].create(
            {"recalculate": True}
        )
        with self.assertRaises(ValidationError):
            model._get_lines_domain()
